...
IS_ROOT = os.getuid() == 0
SUDO = 'pkexec' if (os.environ.get('DISPLAY', False) 
...
def get_root():
    ...
    if '--new' in argv:
        Process(target=lambda: os.execv(
            executable, [executable, __file__, '--root'])
        ).start()
        
        os.abort()
        return

    while cmd([SUDO, executable, __file__, '--new']) != 0: ...
    _exit(0)
